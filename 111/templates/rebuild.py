...
# Read index.html for CSS reference
  ...
  <div class="card-body">
    <details open>
      ...
      <div style="margin-top:8px;padding:12px;background:#f8f9fa;border-radius:6px">
        <p style="font-size:12px;color:#888;margin-bottom:4px">agent.tanyuai.com F12 -> Application -> Cookies -> 全选复制 -> 粘贴 -> 点"从表格粘贴"</p>
...
# Build the full HTML= r"""...""" string for app.py
html_escaped = html.replace('\\', '\\\\').replace('"""', '\\"\\"\\"')
full_html_block = 'HTML = r"""' + html + '\n"""'

# Combine: pre + full_html_block + post
result = pre.decode('utf-8') + full_html_block + '\n' + post.decode('utf-8')
